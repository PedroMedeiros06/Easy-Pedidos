...
export default function TabelaFuncionarios({ funcionarios, setFuncionarios, companyId, onActionSuccess, onEditarClick }) {
  ...
  const tratarBloqueio = async (id, estaAtivo) => {
    ...
    try {
      setProcessandoId(id);
      await adminMembersApi.updateStatus(companyId, id, novoAtivo);
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-sm flex items-center gap-2 text-foreground">
                          ...
                          {["owner", "administrator"].includes(func.member_access) && (
                            <Badge
                              variant="outline"
                              className="gap-1 text-[10px] uppercase text-amber-500 border-amber-500/20 bg-amber-500/10"
                            >
                              <Crown size={10} /> {func.member_access === "owner" ? "Dono" : "Admin"}
                            </Badge>
                          )}
                        </div>
