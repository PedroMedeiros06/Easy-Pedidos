...
export default function ModalCriarFuncionario({ isOpen, onClose, onSuccess, funcionarioParaEditar = null }) {
  ...
  // 🌟 BUSCA DE RESTAURANTES COM URL DINÂMICA CORRETA (/restaurantes)
  useEffect(() => {
    const RESTAURANTES_URL = (import.meta.env.VITE_API_URL || "http://localhost:3000") + "/restaurantes";

    fetch(RESTAURANTES_URL)
      .then((res) => {
        if (!res.ok) throw new Error("Erro na requisição de restaurantes");
        return res.json();
      })
  ...
  // ✅ CORREÇÃO: Removido parâmetro duplicado "restaurants" que quebrava o escopo da função
